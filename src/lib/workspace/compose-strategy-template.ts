...
export type StrategyIntent = (typeof STRATEGY_INTENTS)[number];

export type ComposeStrategyInput = {
...
function strategyNodeFromAdapter(params: {
  ...
  return {
    ...
    data: {
      ...
      protocol: item.protocolLabel ?? item.title,
...
export function buildComposeStrategy({
  intent,
  sourceAmount,
  loopCount,
  targetLTV,
}: ComposeStrategyInput): ComposeStrategyOutput {
